...
export const findMatches = (searchTerm: string): string[] => {
  const normalizedSearchTerm = searchTerm.toLowerCase();
  const matches = new Set<string>();

  // 한글 검색어인 경우
  if (/[가-힣]/.test(searchTerm)) {
    if (koreanToEnglish[searchTerm]) {
      koreanToEnglish[searchTerm].forEach(eng => matches.add(eng.toLowerCase()));
    }
  }
  // 영어 검색어인 경우
  else {
    if (englishToKorean[normalizedSearchTerm]) {
      englishToKorean[normalizedSearchTerm].forEach(kor => matches.add(kor));
    }
  }
